...
int main(){
	...
	//Tem que mudar o valor da variavel quantConsoantes(dentro da função) de -1 para 0.
	//cout<<"Digite qualquer palavra\n";cin>>palavra;
	
	//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	//executando o programa considerando espaços. Tem que mudar o valor da variavel quantConsoantes(dentro da função) de 0 para -1.
